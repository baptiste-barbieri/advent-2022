...
inspectOne : Monkey, Nat, Nat -> {target: Nat, worry: Nat}
inspectOne = \monkey, worry, mod ->
    newWorry = (monkey.op worry) % mod
    {
...
round : List Monkey, Nat, Nat -> List Monkey
round = \monkeys, i, mod ->
    List.get monkeys i
    |> Result.try (\monkey ->
        List.map monkey.items \item -> inspectOne monkey item mod
...
turns : List Monkey, Nat, Nat -> List Monkey
turns = \l, nb, mod ->
    indices l
    |> List.repeat nb
    |> List.joinMap identity
    |> List.walk l \monkeys, index -> round monkeys index mod
...
process = \content ->
    ...
    |> \monkeys -> turns monkeys 10000 (List.map monkeys (\m -> m.test) |> List.walk 1 Num.mul)
    |> score
    |> Num.toStr
